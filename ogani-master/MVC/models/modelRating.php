...
class modelRating extends DB
{
    ...
    public function showRatingproduct($id)
    {
    
        $query = "
            SELECT 
                user.fullname,  -- Lấy fullname từ bảng user
                user.image AS user_Infor,
                Review.rating, 
                Review.comment,
                Review.created_at AS review_date,  -- Thêm ngày đánh giá
                Review.image_url AS review_image,  -- Lấy hình ảnh review (nếu có)
                Review.video_url AS review_video  -- Lấy video review (nếu có)
            FROM 
                Review
            JOIN 
                user ON user.id = Review.user_id  -- Kết nối giữa bảng user và Review để lấy fullname
            WHERE 
                Review.product_id = ?  -- Lọc theo product_id
            ORDER BY 
                Review.created_at DESC  -- Sắp xếp theo ngày đánh giá (mới nhất lên đầu)
            LIMIT 3  -- Lấy 3 bình luận mới nhất
        ";
        ...
        if ($stmt) {
            ...
            if (mysqli_num_rows($result) > 0) {
                $reviews = [];  // Mảng lưu dữ liệu bình luận

                // Duyệt qua các kết quả và lưu vào mảng
                while ($row = mysqli_fetch_assoc($result)) {
                    $reviews[] = $row;
                }

                // In ra kết quả (dùng cho việc debug)
                // print_r($reviews);

                // Trả về mảng chứa các bình luận
                return $reviews;
            } else {
                // Không có kết quả
                return [];
